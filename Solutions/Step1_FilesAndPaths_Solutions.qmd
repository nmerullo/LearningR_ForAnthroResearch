---
title: "Step1 Files And Paths - Solutions"
...
First install the {here} package the same way you insalled the earlier packages: install.packages("here") Here is the documentation if you want to read about it: https://here.r-lib.org/articles/here.html. Now load it in using library(here):
...
The function here from the package also tells us where the project root is. Try that below:

```{r}
here()
```

You can ask the package to tell you why it chose this directory as the project root with dr_here(). Try below:
...
```{r}
here::i_am("Solutions/Step1_FilesAndPaths_Solutions.qmd")
```
...
Let's practice reading in a file from the Data folder using {here} inside a new function, read.csv(). The here function will tell R where to look for the file. **Put the following inside of read.csv(): here("Data", "CRP_IDs.csv").** Note how the subfolder from the working directory and the file are comma separated as if they are separate objects. This is how here() achieves project relative pathing.

Saving it into a variable d will let us view and modify this data frame.

```{r}
d <- read.csv(here("Data", "CRP_IDs.csv"))
```

What does d look like? **Use head() with the variable d inside the parentheses.**
