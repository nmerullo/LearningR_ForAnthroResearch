---
title: "Step4 Basic Reformatting of Files - Part 1 - Solutions"
...
print(paste("Number of rows in IgE:", nrow(ige))) #let's us see how many rows there are
colnames(ige)
```
...
print(paste("Number of rows in Il-6:", nrow(il6))) #let's us see how many rows there are
colnames(il6)
...
We also need to remove a sample ID that is XXX

```{r}
ige <- ige |> filter(!str_detect(Sample_ID, "XXX"))
```
...
```{r}
ige <- ige |> unite(ID, ID, Date.of.measurement.Blood, sep = "-", remove = FALSE)
head(ige)
```
...
Note that after the "_selected" object is made, they have twice as many rows as before.

#### 4. Pivot wide again
To make it so that each ID is a row!

```{r}
ige_selected <- ige_selected %>%
  group_by(ID) %>%
  mutate(rep = c("a", "b")) %>%  # label first and second row per ID
  ungroup() %>%
  pivot_wider(
...
And then back to 299 

#### 5. Remove the Crazy ID row
That we created to help make the above step easier

```{r}
ige_selected <- ige_selected |> select(-ID)
```
...
```{r}
il6 <- il6 |> unite(ID, ID, Date.of.measurement.Blood, sep = "-", remove = FALSE)
head(il6)
```
...
il6_selected <- il6 %>%
  # Step 1: Identify the best run (lowest CV per row)
  mutate(
    best_run = pmap_chr(select(., CV_1, CV_2), ~ {
      cvs <- c(...)
      run_index <- which.min(replace(cvs, is.na(cvs), Inf))
      if (length(run_index) == 0) NA_character_ else as.character(run_index)
    })
...
```{r}
il6_selected <- il6_selected %>%
  group_by(ID) %>%
  mutate(rep = c("a", "b")) %>%  # label first and second row per ID
  ungroup() %>%
  pivot_wider(
    id_cols = c(ID, Short.ID, Date.of.measurement.Blood, Plate_FirstRun, Plate_SecondRun, best_run, Well.ID, Count, Mean, Std.Dev, CV),
    names_from = rep,
    values_from = c(Well, Conc.Dil, X450, Conc),
    names_glue = "{.value}_{rep}"
  )
...
```{r}
il6_selected <- il6_selected |> select(-ID)
```
...
```{r}
write.csv(ige_selected, here("Data", "UCHP_InflammationBiomarkers-IgE_Compiled.csv"), row.names = FALSE)
```

```{r}
write.csv(il6_selected, here("Data", "UCHP_InflammationBiomarkers-Il-6_Compiled.csv"), row.names = FALSE)
```
