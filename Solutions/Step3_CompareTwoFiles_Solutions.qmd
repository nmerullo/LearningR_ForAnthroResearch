---
title: "Step3 Compare Two Files - Solutions"
...
Important note! The tidyverse family of functions introduces the pipe operator %>% which is super helpful for chaining together a lot of functions! You'll see me use this in the example code, but it's important to know what it does too. It takes whatever is on the left side of the pump and feeds it into the first argument of the right side of the pipe. There are a lot of implications for this but mostly just helps us move output quickly without having to save an intermediary to an object we don't need.
...
```{r}
#IgE
ige_ids <- read.csv(here("Data", "IgE_IDS.csv"))
ige_results <- read.csv(here("Data", "UCHP_InflammationBiomarkers_IgE.csv"), na.strings = c("", "NA", "na")) %>% filter(!is.na(Sample_ID))

#Il-6
il6_ids <- read.csv(here("Data", "Il-6_IDS.csv"))
il6_results <- read.csv(here("Data", "UCHP_InflammationBiomarkers_Il-6.csv"), na.strings = c("", "NA", "na")) %>% filter(!is.na(Sample_ID))
```
...
```{r}
## IgE
cat("Master list of IgE IDs Number of Rows:", nrow(ige_ids), "\n")
cat("IgE results Rows:", nrow(ige_results), "\n")
cat("Difference between number of rows we expect (master list) and what we have so far (results):", nrow(ige_ids)-nrow(ige_results), "\n")


## Il-6
cat("Master list of Il-6 IDs Number of Rows:", nrow(il6_ids), "\n")
cat("Il-6 results Rows:", nrow(il6_results), "\n")
cat("Difference between number of rows we expect (master list) and what we have so far (results):", nrow(il6_ids)-nrow(il6_results), "\n")
```
...
```{r}
#IgE
ige.names <- setdiff(ige_ids$SampleID, ige_results$Sample_ID)
head(ige.names) #showing the first 6 entries
class(ige.names) #what is the class of this vector?
cat("Number of names not present in data entry:", length(ige.names), "\n")

setdiff(ige_results$Sample_ID, ige_ids$SampleID)


#Il-6
il6.names <- setdiff(il6_ids$SampleID, il6_results$Sample_ID)
head(il6.names) #showing the first 6 entries
class(il6.names) #what is the class of this vector?
cat("Number of names not present in data entry:", length(il6.names), "\n")

setdiff(il6_results$Sample_ID, il6_ids$SampleID)
```
